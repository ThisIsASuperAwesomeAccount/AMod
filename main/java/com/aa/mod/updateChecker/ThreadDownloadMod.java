...
public class ThreadDownloadMod extends Thread {
	...
	public ThreadDownloadMod(String fileName) {
		setName("Arcane Arteries Download File Thread");
	...
	public void run() {
		try {
			...
			if(Minecraft.getMinecraft().thePlayer != null)
				Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentTranslation("botania.versioning.doneDownloading", fileName).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.AQUA)));
